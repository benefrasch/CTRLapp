...
namespace CTRLapp.Views.SettingsPages.GUI
{
    ...
    public partial class GuiPage : ContentPage
    {
        ...
        public GuiPage(int master_menu, int bottom_menu)
        {
            InitializeComponent();
        ...
        {
            if (Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects == null) Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects = new List<Objects.Object>();
            int obj_index = Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects.Count;
            Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects.Add(temp);
        ...
        private void Initialize_Objects() //loads existing objects
        {
            ...
            for (int obj_index = 0; obj_index < Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects.Count; obj_index++)
            {
                Load_Object(obj_index);
            }
        }
        ...
        private void Load_Object(int obj_index) //object with pan and tap for config
        {
            ...
            var grid = new ObjectView(master_menu, bottom_menu, obj_index)
            {
                IsEnabled = false,
                TranslationX = obj.X,
                TranslationY = obj.Y,
            ...
            {
                Navigation.PushModalAsync(new ObjectPage(master_menu, bottom_menu, obj_index));
            };
            ...
            panGestureRecognizer.PanUpdated += (s, e) =>
            {
                ...
                switch (e.StatusType)
                {
                    ...
                    case GestureStatus.Running:
                        grid.TranslationX = ((int)(Math.Round(Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects[obj_index].X + e.TotalX) / (double)rounding_value)) * rounding_value;
                        grid.TranslationY = ((int)(Math.Round(Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects[obj_index].Y + e.TotalY) / (double)rounding_value)) * rounding_value;
                        break;
                    case GestureStatus.Completed:
                        ...
                        Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects[obj_index].X = (int)grid.TranslationX;
                        Variables.Variables.Layout[master_menu].BottomMenuItems[bottom_menu].Objects[obj_index].Y = (int)grid.TranslationY;
